...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    ...
    if (projects.length === 0) {
      // 경우 2) 프로젝트 없음 → 프로젝트 생성
      router.push("/project/new");
    } else {
      // 경우 3) 프로젝트 1개 이상 → 태스크 관리
      router.push(`/project/${projects[0].id}/tasks`);
    }
  };

  return (
    <div className="flex min-h-screen flex-col">
      <header className="flex h-14 items-center justify-end border-b border-line bg-white px-6 text-sm font-semibold text-ink-soft">
        인수인계 태스크 관리
      </header>
